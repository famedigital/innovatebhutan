 ...
 *
 * Usage: npx tsx scripts/import-amc-final.ts
 */
...
async function importAMCClients() {
  ...
  try {
    ...
    for (let i = 0; i < rawData.length; i++) {
      ...
      // Create AMC contract
      const licenseKeyNote = rancelabCode ? `\nLicense Key: ${ralcodelabCode}` : '';
      const notes = `Software Edition: ${softwareEdition}${industry ? `\nIndustry: ${industry}` : ''}${licenseKeyNote}`;
