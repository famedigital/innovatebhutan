...
function cleanPhoneNumber(phone) {
  ...
  // Truncate to 50 chars max (database constraint)
...
rawData.forEach((row, i) => {
  ...
  let contactPerson = row["Owner's Name "] || null;
  if (contactPerson && contactPerson.length > 255) contactPerson = contactPerson.substring(0, 255);
  ...
  let industry = row['Nature Of Business '] || null;
  if (industry && industry.length > 100) industry = industry.substring(0, 100);
  let licenseKey = (row['License Key '] || '').trim() || null;
  // Truncate license key to 50 characters max (database constraint)
  if (licenseKey && licenseKey.length > 50) licenseKey = licenseKey.substring(0, 50);
  const licenseUrl = row['URL '] || null;
  ...
  // Client INSERT with ON CONFLICT to handle duplicates
  ...
  sql += escapeString(licenseKey) + ', ';
  sql += escapeString(licenseUrl) + ', ';
  sql += escapeString(industry) + ', ';
  sql += "'" + tier + "', 'Bhutan', 'Asia/Thimphu', 'whatsapp', ";
  sql += (startDate ? "'" + startDate + "'" : 'NOW()') + ', NOW()) ';
  sql += 'ON CONFLICT (ralcodelab_code) DO NOTHING;';
  ...
  if (licenseKey) notes += ' | License: ' + licenseKey;

  // AMC INSERT - use subquery to find client_id
  // Try to find by ralcodelab_code first, then by name if no code
  let clientQuery;
  if (licenseKey) {
    clientQuery = '(SELECT "id" FROM "clients" WHERE "ralcodelab_code" = ' + escapeString(licenseKey) + ' LIMIT 1)';
  } else {
    clientQuery = '(SELECT "id" FROM "clients" WHERE "name" = ' + escapeString(businessName) + ' ORDER BY "id" DESC LIMIT 1)';
  }
...
console.log('-- IMPORTANT: After running the client INSERTs, update AMC client_id values to match actual database IDs');
