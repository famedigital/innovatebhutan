    ...
    <div className="h-full flex flex-col bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
      ...
      <div className="p-4 border-b border-zinc-200 dark:border-zinc-800">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm text-foreground">Your Selection</h3>
          </div>
          {cart.length > 0 && (
            <button
              onClick={onClear}
        ...
        {cart.length > 0 && (
          <p className="text-xs text-foreground/50 mt-1">{cart.length} item{cart.length > 1 ? 's' : ''}</p>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        <AnimatePresence>
          {cart.length === 0 ? (
            ...
            >
              <ShoppingCart className="w-12 h-12 text-foreground/20 mb-3" />
              <p className="text-sm text-foreground/40">Your cart is empty</p>
              <p className="text-xs text-foreground/30 mt-1">Add services to get started</p>
            </motion.div>
          ) : (
            cart.map((item, index) => {
              ...
              return (
                <motion.div
                  ...
                  className="flex items-center gap-3 p-3 rounded-xl bg-zinc-50 dark:bg-zinc-800/50 group"
                >
                  <div
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.subService && (
                      <p className="text-xs text-foreground/50 truncate">{item.subService}</p>
                    )}
                  </div>
                  <motion.button
                    ...
                    className="w-8 h-8 rounded-full flex items-center justify-center text-foreground/40 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all opacity-0 group-hover:opacity-100"
                  >
                    <X className="w-4 h-4" />
                  </motion.button>
                </motion.div>
      ...
      <div className="p-4 border-t border-zinc-200 dark:border-zinc-800">
        <a
          ...
          className={`w-full py-4 rounded-2xl font-medium transition-all flex items-center justify-center gap-2 ${
            cart.length > 0
              ...
              : 'bg-zinc-100 dark:bg-zinc-800 text-foreground/40 cursor-not-allowed'
          }`}
          onClick={(e) => {
        ...
        >
          ...
          {cart.length > 0 ? 'Contact on WhatsApp' : 'Select Services'}
        </a>
      </div>
    </div>
  );
      ...
      <div className="bg-white dark:bg-zinc-900 border-b border-border">
        <div className="max-w-7xl mx-auto px-6 py-12">
          ...
          <p className="text-foreground/50 mt-2">
            {selectedService ? 'Select and customize your services' : 'Premium solutions for your business'}
          </p>
        </div>
      </div>
